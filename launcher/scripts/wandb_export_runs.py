...
# %%
def get_runs(entity: str, project: str, run_name: str) -> list[Run]:
    api = wandb.Api()
    runs = api.runs(f"{entity}/{project}")
    runs: list[Run] = [run for run in runs if run_name in run.name]
    return runs

# def export_run_data(entity: str, project: str, run_name: str, num_samples=None):
#     runs = get_runs(entity, project, run_name)
#     run_dict = summarize_runs(runs, num_samples)
#     return run_dict

def summarize_run_data(df: pd.DataFrame, col_order=DEFAULT_COL_ORDER):
    summary_keys = list(set(df.columns) - set(["history", "metadata"]))
