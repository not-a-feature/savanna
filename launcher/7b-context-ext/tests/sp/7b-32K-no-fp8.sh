...
MODEL_CONFIG=$SAVANNA_ROOT/configs/7b-context-ext/model_configs/tests/sp/7b-$ROPE_SCALE-$CONTEXT_LEN-no-fp8.yml
...
LAUNCHER=srun
JOBTIME="01:00:00"
SUFFIX="$ROPE_SCALE-$CONTEXT_LEN-no-fp8"
...
OVERRIDES="sequence_parallel=True partition-activations=False"
RUN_CFG_STR=`tr ' ' '__' <<< $OVERRIDES`
RUN_CFG_STR=`tr '=' '-' <<< $RUN_CFG_STR`

for N in "${NODES[@]}"; do
    NUM_NODES=$N 
    RUN_NAME=n$N-$RUN_CFG_STR
    JOB_NAME=$BASE_NAME-$RUN_NAME-no-fp8
