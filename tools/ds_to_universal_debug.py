...
def extract_zero_shards(dir, ds_checkpoint, indices_3D):
    ...
    for param_group_id in range(param_groups_cnt):
        ...
        for name, fragment_mapping in param_slice_mappings[param_group_id].items():
            ...
            print(f"dpt{dp_index}{pp_index}{tp_index} {param_group_id} {name} => {fragment_mapping.start}:{fragment_mapping.numel}")
...
def main(args):
    ...
    if zero_stage <= 2:
        ...
        print(f"*** Found {ds_checkpoint.tp_degree} TP and {ds_checkpoint.pp_degree} PP ranks")
        
        print(f"{checkpoint_paths=}")
        print(f"mp rank files: {ds_checkpoint.mp_rank_files}")
