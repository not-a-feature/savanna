...
def concatenate_tensors_across_shards(
    ...
    if partition_dim is None:
        ...
        print(f"tensor {tensor_name} not partitioned, returning rank0 tensor {tensors[0].shape}")
        return tensors[0]
...
def check_params(detected, expected, buffers, param_pattern=DEFAULT_PARAM_PATTERN, verbose=False):
    ...
    extra_params = [param for param in extra_params if not any(param in buffer for buffer in buffers)]
    ...
    #     print("WARNING:Missing or extra params detected!")

def convert(input_data_shards, output_data_shards, model_parameter_names: List[str], param_list: List[Param], buffers: List[str], verbose=False, exclude_extra=False):
...
def main(args, param_list: List[Param] = EVO2_PARAMS):
    ...
    sharded_model_state = torch.load(args.zero3_model_state, map_location=DEVICE)
    sharded_param_shapes = sharded_model_state['param_shapes']
    buffers = sharded_model_state['buffer_names']
    ...
    check_params(detected=model_parameter_names, expected=param_names, buffers=buffers, verbose=args.verbose)
    # Convert the checkpoint
    convert(input_data_shards, output_data_shards, model_parameter_names, param_list, buffers=buffers, verbose=args.verbose, exclude_extra=args.exclude_extra)
    print("Done!")
...
def get_args():
    ...
    parser.add_argument("--zero3_model_state", type=str, required=True, help="Path to the zero3 model state file")
